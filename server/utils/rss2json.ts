...
export async function rss2json(url: string, options: any = {}): Promise<RSSInfo | undefined> {
  if (!/^https?:\/\/[^\s$.?#].\S*/i.test(url)) return

  const data = await myFetch(url, { ...options, responseType: "text" as any }) as string
  ...
  if (!channel && rdf) channel = rdf.channel
  if (Array.isArray(channel)) channel = channel[0]

  function readLink(link: any) {
    if (Array.isArray(link)) {
      const item = link.find(i => i.rel === "alternate") ?? link.find(i => !i.rel) ?? link[0]
  ...
  const rss = {
    ...
    link: readLink(channel.link),
    image: channel.image ? channel.image.url : channel["itunes:image"] ? channel["itunes:image"].href : "",
    category: channel.category || [],
    updatedTime: channel.lastBuildDate ?? channel.updated ?? channel["dc:date"],
    items: [],
  }
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const obj = {
      ...
      link: readLink(val.link),
      author: val.author && val.author.name ? val.author.name : val["dc:creator"],
      created: val.updated ?? val.pubDate ?? val.created ?? val["dc:date"],
